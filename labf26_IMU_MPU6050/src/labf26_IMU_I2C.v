module I2C_V1_3#(parameter CLK_FREQ = 27000000,
...
always@(posedge i_clk or posedge i_rst)begin // chuyển trạng thái
    if(i_rst)begin
        ...
        if(i_start && (state == IDLE)) state <= START; // chỉ được start khi rảnh
        if ((tick == TICK3) && xong_tick) begin // chỉ đổi trạng thái tại clock cuối tick3
            state <= next_state;
        end
    end
end
...
always@(*)begin // logic chuyển trạng thái
    ...
    DATA_ACK: begin
        if(tick == TICK3)begin
            if(o_ack_error == 0)begin
                if(i_cmd_repeat)begin
                    next_state = REPEATED_START;
                end
                else if(i_cmd_stop)begin
                    next_state = STOP;
                end
                else begin
                    next_state = IDLE;
    ...
    MASTER_ACK: begin
        if(tick == TICK3)begin
            next_state = (i_cmd_stop)? STOP: READ_DATA;
        end
    end
...
always@(posedge i_clk or posedge i_rst)begin // logic đầu ra
        ...
        if(i_start && (state == IDLE || state == ADDR_ACK || state == DATA_ACK))begin
